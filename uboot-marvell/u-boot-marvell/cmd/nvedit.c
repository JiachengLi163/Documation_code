			  ...
			  int argc, char * const argv[])
{
	...
	int function = CONFIG_DEFAULT_BOOT_DEVICE;
	struct eeprom_struct* cfg= cfg_eeprom_get_board_config();
// env default not append -e -s -u ,then the default will check from record info
...
#ifdef CONFIG_CMD_MVEBU_HW_INFO
		set_default_env_by_mode("## Resetting to default environment\n", function);
#else
