...
static int initr_env(void)
{
	...
	// check is need load info from emmc
#ifdef CONFIG_BOARD_CONFIG_EEPROM
	struct eeprom_struct* cfg = cfg_eeprom_get_board_config();
	if (env_get_yesno("force_load") != 0 || cfg->forceReload) {
		switch (cfg->bootdev[0]) {
		case 'E':
			set_default_env_by_mode("!force load default setting", ENV_EMMC);
			break;
		case 'S':
			set_default_env_by_mode("!force load default setting", ENV_SD);
			break;
		case 'U':
			set_default_env_by_mode("!force load default setting", ENV_USB);
			break;
		case 'N':
			set_default_env_by_mode("!force load default setting", ENV_NFS);
			break;
		default:
			set_default_env_by_mode("!force load default setting",
			CONFIG_DEFAULT_BOOT_DEVICE);
			break;
		}
		...
		if (hw_param_num > MVEBU_HW_INFO_LEN) {
			pr_err("variables from EEPROM can not exceed %d\n",
					MVEBU_HW_INFO_LEN);
			return 0;
		}

		/* save the HW parameter to ENV variables one by one */
		for (idx = 0; idx < hw_param_num; idx++) {
			env_set(hw_info_data_array[idx].name,
					hw_info_data_array[idx].value);
		}
	}
#endif // CONFIG_BOARD_CONFIG_EEPROM
