		       ...
		       int of_size, struct lmb *lmb)
{
	...
	int ret = -EPERM;
	int fdt_ret;
	char *boot_initramfs = env_get("boot_initramfs");
#ifdef CONFIG_CMD_MVEBU_HW_INFO
	int node, hw_param_num, idx;
#endif //CONFIG_CMD_MVEBU_HW_INFO
	...
	if (boot_initramfs && !strcmp(boot_initramfs, "1")) {
		node = fdt_find_or_add_subnode(blob, 0, "chosen");
		if (node >= 0) {
			fdt_setprop_string(blob, node, "bootargs", "ramdisk_size=100000000 rw rootwait earlycon=uart8250,mmio32,0xff1a0000 swiotlb=1 console=ttyFIQ0 rootfstype=ext4 coherent_pool=1m");
		}
	}
...
#ifdef CONFIG_CMD_MVEBU_HW_INFO
	/* find or create "/hwinfo" node */
	node = fdt_find_or_add_subnode(blob, 0, "hwinfo");
	if (node >= 0)
	{
		fdt_setprop_string(blob, node, "U_BOOT_VERSION", U_BOOT_VERSION);
		fdt_setprop_string(blob, node, "CC_VERSION_STRING", CC_VERSION_STRING);
		fdt_setprop_string(blob, node, "LD_VERSION_STRING", LD_VERSION_STRING);
		...
		if (hw_param_num != 0 && hw_param_num <= MVEBU_HW_INFO_LEN) {
			/* save the HW parameter to ENV variables one by one */
			for (idx = 0; idx < hw_param_num; idx++)
				fdt_setprop_string(blob, node, hw_info_data_array[idx].name,
