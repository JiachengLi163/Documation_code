...
int genphy_update_link(struct phy_device *phydev)
{
	...
	if ((phydev->autoneg == AUTONEG_ENABLE) &&
		...
		while (!(mii_reg & BMSR_ANEGCOMPLETE)) {
			...
			udelay(1000);	/* 1 ms */
			if(0x1410EA2 == phydev->phy_id)
				phy_write(phydev, MDIO_DEVAD_NONE, 0x16,0); // page 0 for rj45
			mii_reg = phy_read(phydev, MDIO_DEVAD_NONE, MII_BMSR);
			if(!(mii_reg&BMSR_ANEGCOMPLETE) && 0x1410EA2 == phydev->phy_id )
			{
				phy_write(phydev, MDIO_DEVAD_NONE, 0x16,1); // page 1 for sfp
				mii_reg = phy_read(phydev, MDIO_DEVAD_NONE, MII_BMSR);
			}
		}
		printf(" done\n");
					     ...
					     phy_interface_t interface)
{
	...
	while (phy_mask) {
		...
		if (!strncmp(bus->name, "cp0-mdio",8)) {
			int val = readl(0xf212a204);
			val &= 0xFFFFFFFE;
			writel(val, 0xf212a204); 
		}
