...
#define MIIM_88E151x_LED_POLARITY_CTRL	17
...
/* Marvell 88E1543 */
static int m88e1543_config(struct phy_device *phydev)
{
	struct mii_dev *bus = phydev->bus;

	// configure the PHY

	// hw patch, phy config3 set to 0111
	bus->write(bus, phydev->addr + 1, MDIO_DEVAD_NONE, 0x16, 0x0004); // page 4
	bus->write(bus, phydev->addr + 1, MDIO_DEVAD_NONE, 0x1A, 0xB002);
	phy_write(phydev, MDIO_DEVAD_NONE, 0x16, 0x0004); // page 4
	phy_write(phydev, MDIO_DEVAD_NONE, 0x1A, 0xB002);

	// On CPU, SGMII to QSGMII
	bus->write(bus, phydev->addr + 1, MDIO_DEVAD_NONE, 0x16, 0x0012); // page 18 cpu eth
	bus->write(bus, phydev->addr + 1, MDIO_DEVAD_NONE, 0x14, 0x8005); // SGMII(system) to QSGMII(media)
	udelay(1000);    // 1 ms

	bus->write(bus, phydev->addr + 1, MDIO_DEVAD_NONE, 0x16, 0x0004); // page 4
	bus->write(bus, phydev->addr + 1, MDIO_DEVAD_NONE, 0x00, 0x8140);
	udelay(5000);    // 5 ms
	bus->write(bus, phydev->addr + 1, MDIO_DEVAD_NONE, 0x1B, 0x7C03);
	bus->write(bus, phydev->addr + 1, MDIO_DEVAD_NONE, 0x16, 0x0000); // page 0

	// On Media,  QSGMII to Auto Media Detect
	phy_write(phydev, MDIO_DEVAD_NONE, 0x16, 0x0012); // page 18 RJ45 + SFP
	phy_write(phydev, MDIO_DEVAD_NONE, 0x14, 0x8007); // QSGMII(System) to Auto Media detect (Copper/1000base-x)
	udelay(1000);    // 1 ms
	phy_write(phydev, MDIO_DEVAD_NONE, 0x16, 0x0004); // page 4
	phy_write(phydev, MDIO_DEVAD_NONE, 0x00, 0x9140);
	udelay(5000);    // 5 ms
	phy_write(phydev, MDIO_DEVAD_NONE, 0x1B, 0x7C03);

	// led config
	phy_write(phydev, MDIO_DEVAD_NONE, 0x16, 0x0003); // page 3
	phy_write(phydev, MDIO_DEVAD_NONE, 0x10, 0x0177);
	phy_write(phydev, MDIO_DEVAD_NONE, 0x11, 0x0010);
	phy_write(phydev, MDIO_DEVAD_NONE, 0x16, 0x0000); // page 0

	// force enable sfp tx_disabled

	gpio_request(53,"eth4_tx_disabled");
	gpio_direction_output(53 ,1); // cn1 and cn8 eth4

	gpio_request(74,"eth5_tx_disabled");
	gpio_direction_output(74 ,1); // cn11 and cn12 eth5

	phy_write(phydev, MDIO_DEVAD_NONE, 0x16, 0x0001); // page 4
	phy_write(phydev, MDIO_DEVAD_NONE, 0x04, 0x0060);
	phy_write(phydev, MDIO_DEVAD_NONE, 0x16, 0x0000); // page 0
...
static int m88e1518_config(struct phy_device *phydev)
{
	...
	/* Select page 3 */
	phy_write(phydev, MDIO_DEVAD_NONE, MIIM_88E1118_PHY_PAGE, 3);
/*
	 _____________
	|             |
	|             |
...
*/
	...
	//                                 LED[0]      LED[1]
	//--------------------------------------------------------
	//1000 Mbps Link - No Activity     On          Off	green
	//1000 Mbps Link - Activity        Blink       Off	green
	//100  Mbps Link - No Activity     Off         On	yellow
	//100  Mbps Link - Activity        Off         Blink	yellow
	//10   Mbps Link - No Activity     On          On	green + yello
	//10   Mbps Link - Activity        Blink       Blink	green + yello
	//No   link                        Off         Off
	//--------------------------------------------------------


	//15:12 reserved (default)
	//11:8  Led2 (not used)(default)
	//7:4   Led1 (ACT))(default On - Link, Blink - Activity, Off - No Link)
	//3:0   Led0 F (LINK 10/100/1000 Mbps)(MODE 4)
	phy_write(phydev, MDIO_DEVAD_NONE, MIIM_88E151x_LED_FUNC_CTRL, 0x101F);

	//15:12 Led1 = 100% mixing
	//11:8  Led0 = 100% mixing
	//7:6   reserved (default)
	//5:4   Led2 : (not used)(default)
	//3:2   Led1 : drive low = On
	//1:0   Led0 : drive low = On
	phy_write(phydev, MDIO_DEVAD_NONE, MIIM_88E151x_LED_POLARITY_CTRL, 0x8800);

	//15    Force INT = normal operation
	//14:12 Pulse stretch duration 170 ms to 340 ms
	//11    INTn active low
	//10:8  Blink Rate 340 ms
	//7     Interrupt Enable (default)
	//6:4   reserved (default)
	//3:2   Speed Off Pulse Period 340ms
	//1:0   Speed On  Pulse Period 340ms
	phy_write(phydev, MDIO_DEVAD_NONE, MIIM_88E151x_LED_TIMER_CTRL, 0x4b0a);

	//change to page 0 to control normal
...
static int m88e2110_config(struct phy_device *phydev)
{
	...
	/*
         ...
         |               |
         |_______________|
	left Yellow 10M/10M and blink when have data Transmit
	right Yellow 1G and blink when have data Transmit
	right Green 2.5G and blink when have data Transmit
	*/
	/* ACT Leds
	 * Transmit or Receive Activity		blink
	 * Copper link  					Solid
	 * */
	phy_write(phydev, 0x1F, 0xF020, 0x160); // 10Mbps or 100Mbps and act
	phy_write(phydev, 0x1F, 0xF021, 0x1a8); // 2.5G Green and act
	phy_write(phydev, 0x1F, 0xF022, 0x150); // 1G Yellow and act
