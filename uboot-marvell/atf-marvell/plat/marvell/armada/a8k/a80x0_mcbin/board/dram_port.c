...
#define MVEBU_CP_MPP_CTRL2_OFFS		8
#define MVEBU_CP_MPP_CTRL3_OFFS		12
#define MVEBU_CP_MPP_CTRL2_I2C1_SCK_ENA	0x7
#define MVEBU_CP_MPP_CTRL3_I2C1_SDA_ENA	0x7
...
{
	uint32_t val;
	uintptr_t reg = MVEBU_CP_MPP_REGS(1, 0);

	/* configure CP1 MPP 2 and 3 to i2c */
	val = mmio_read_32(reg);
	val &= ~((MVEBU_MPP_CTRL_MASK << MVEBU_CP_MPP_CTRL2_OFFS) |
		(MVEBU_MPP_CTRL_MASK << MVEBU_CP_MPP_CTRL3_OFFS));
	val |= (MVEBU_CP_MPP_CTRL2_I2C1_SCK_ENA << MVEBU_CP_MPP_CTRL2_OFFS) |
		(MVEBU_CP_MPP_CTRL3_I2C1_SDA_ENA << MVEBU_CP_MPP_CTRL3_OFFS);
...
void plat_marvell_dram_update_topology(void)
{
	...
	if (tm->cfg_src == MV_DDR_CFG_SPD) {
		...
		mpp_config();
		/* initialize the i2c */
		i2c_init((void *)MVEBU_CP1_I2C_BASE);
